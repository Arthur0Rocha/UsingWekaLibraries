	...
	public static ASSearch[] initializeSearchVector(String[] input){
		...
		for(String s : input)
			...
			if(s.equals("PSO")){
				...
				evo.setCrossoverProbability(0.8); 	// Padrão 0.6
				evo.setGenerations(100); 			// Padrão 20
				evo.setMutationProbability(0.1); 	// Padrão 0.01
				evo.setPopulationSize(32);			// Padrão 20
				...
				//rank.setGenerateRanking(true); // Padrão true
				rank.setNumToSelect(numToSelect); // Padrão -1
				//rank.setThreshold(-1.7976931348623157E308); // Padrão -1.7976931348623157E308
				output[i++] = rank;
